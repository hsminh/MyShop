...
public class ProductCategoriesSerVice {
    ...
    @Transactional
    public Product_categories DeleteCategory(Integer id) throws Product_categoriesException {
        ...
        if (categoryOptional.isPresent()) {
            ...
            if(category.getLiProducts().size()!=0)
            {
                System.out.println("come he");
                    throw new Product_categoriesException("Category with ID " + id + "contains products, cannot delete.");
            }
            this.productCategoryRepository.delete(category);
            return category;
        } else {
            throw new Product_categoriesException("Category with ID " + id + " not found");
        }
    }

    public Product_categories FindById(Integer id) throws Product_categoriesException {
        Optional<Product_categories> categoryOptional = this.productCategoryRepository.findById(id);
        if (categoryOptional.isPresent()) {
            Product_categories category = categoryOptional.get();
            return category;
    ...
    public String CheckNameAndSlugUnique(Integer id, String name, String slug) throws Product_categoriesException {
        if (id == null || id == 0) {
